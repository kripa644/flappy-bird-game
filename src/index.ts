...
const updateGame = (bird: number, pipes: PipeProps[][]) =>
  (game => (
    pipes.forEach((col: PipeProps[]) => col.forEach((v: PipeProps) => (game[v.x][v.y] = 2))),
    (game[bird][0] = 1),
    game
  ))(
    Array(gameSize)
...
const valueOnCollisionFor = (pipes: PipeProps[][]) => ({
      ...
      ((state, [bird, pipes]) => ({
        ...
        lives:
          ...
          valueOnCollisionFor(pipes).when(
            pipes.some((c: PipeProps[]) => c.some((c: PipeProps) => c.y === 0 && c.x === bird))
          ),
        score:
